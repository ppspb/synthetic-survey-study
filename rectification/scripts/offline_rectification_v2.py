...
def geometric_ratio(model_sum, human_sum, eps=1e-4):
    ...
    r = h / m
    # center to avoid pure scale inflation; geometric mean = 1
    gm = math.exp(float(np.log(r).mean()))
    return r / gm


def fit_bias_tables(train: pd.DataFrame, eps: float = 1e-4, shrink: float = 5.0):
    # aggregate sums by (n_options), (category,n_options), (group,n_options)
    glob = {}
    cat = {}
    grp = {}
    ...
    grp_stats = defaultdict(lambda: {'m': None, 'h': None, 'count': 0})
    ...
    for _, r in train.iterrows():
        ...
        upd(grp_stats, (r['GROUP_KEY'], n), p, h)
...
def correct_dist(row, glob, cat, grp, lg=1.0, lc=1.0, lp=0.5):
    ...
    rp = np.array(grp.get((row['GROUP_KEY'], n), np.ones(n)), dtype=float)
    if len(rg) != n: rg = np.ones(n)
    if len(rc) != n: rc = np.ones(n)
    if len(rp) != n: rp = np.ones(n)
    w = p * (rg ** lg) * (rc ** lc) * (rp ** lp)
    s = float(w.sum())
...
def apply_bias_tables(cells: pd.DataFrame, glob, cat, grp, lg, lc, lp) -> pd.DataFrame:
    out = cells.copy()
    out['corrected_dist'] = out.apply(lambda r: correct_dist(r, glob, cat, grp, lg=lg, lc=lc, lp=lp), axis=1)
    ...
    out['corrected_entropy'] = out['corrected_dist'].apply(lambda p: float(-(np.array(p) * np.log(np.clip(np.array(p), 1e-12, 1.0))).sum()))
    return out
...
def tune_weights(train: pd.DataFrame, glob, cat, grp):
    grid_g = [0.0, 0.5, 1.0]
    grid_c = [0.0, 0.5, 1.0, 1.5]
    grid_p = [0.0, 0.25, 0.5, 0.75]
    best = None
    best_score = float('inf')
    for lg in grid_g:
        for lc in grid_c:
            for lp in grid_p:
                pred = apply_bias_tables(train, glob, cat, grp, lg, lc, lp)
                score = float(pred['corrected_w1'].mean())
                if score < best_score:
                    best_score = score
                    best = (lg, lc, lp)
...
def main():
    ...
    (lg, lc, lp), calib_score = tune_weights(train, glob, cat, grp)
    corrected = apply_bias_tables(cells, glob, cat, grp, lg, lc, lp)
    cal_corr = corrected[corrected['split'] == 'calibration'].copy()
    eval_corr = corrected[corrected['split'] == 'evaluation'].copy()

    summary = {
        'method': 'option_level_family_group_rectification_v2',
        'shrink': args.shrink,
        'weights': {'global': lg, 'category': lc, 'group': lp},
        'calibration_tuned_mean_w1': calib_score,
    ...
    save_cells.to_csv(out_dir / 'cell_level_rectification_v2.tsv', sep='\t', index=False)
    category_eval.to_csv(out_dir / 'category_rectification_eval_v2.tsv', sep='\t', index=False)
    question_eval.to_csv(out_dir / 'question_rectification_eval_v2.tsv', sep='\t', index=False)
    group_eval.to_csv(out_dir / 'group_rectification_eval_v2.tsv', sep='\t', index=False)
    json.dump(summary, open(out_dir / 'summary_v2.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=2)

    with open(out_dir / 'README_v2.txt', 'w', encoding='utf-8') as f:
        f.write('Option-level family/group-aware rectification v2\n')
