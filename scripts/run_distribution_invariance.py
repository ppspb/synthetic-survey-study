...
logger = logging.getLogger('distribution_invariance')
MAX_OPTION_COL = 10
BASE_FIELDS = [
    'QUESTION_ID','PARTICIPANT_ID','GROUP_KEY','Continent','Urban_Rural','Education',
    'MODEL_SCORE','MOST_LIKELY_ANSWER','DECLARED_ENTROPY','prompt_mode','perturbation','model','HUMAN_SCORE'
]
...
def setup_logging(run_dir: Path) -> None:
    ...
    log_path = log_dir / f"invariance_{time.strftime('%Y%m%d_%H%M%S')}.log"
...
def sample_score(option_probs: dict[str, float], participant_id: str, qid: str, mode: str, perturbation: str, seed_base: int = 17) -> int:
    keys = sorted(option_probs.keys(), key=lambda x: int(x))
    probs = np.array([option_probs[k] for k in keys], dtype=float)
    probs = probs / probs.sum()
    digest = hashlib.sha256(f'{participant_id}|{qid}|{mode}|{perturbation}|{seed_base}'.encode('utf-8')).hexdigest()
...
def existing_completed_keys(predictions_path: Path) -> set[tuple[str,str,str,str]]:
    if not predictions_path.exists():
        return set()
    try:
        df=robust_read_predictions(predictions_path)
        return set(zip(df['QUESTION_ID'].astype(str), df['PARTICIPANT_ID'].astype(str), df['prompt_mode'].astype(str), df['perturbation'].astype(str)))
    except Exception as e:
...
def reverse_anchor_order(text: str) -> str:
    # swap the order in "1 meaning 'A' and 4 meaning 'B'"
    pattern = re.compile(r"(On a scale of\s+[-\d]+\s+to\s+[-\d]+,\s*)([-\d]+)\s+meaning\s+'([^']+)'\s+and\s+([-\d]+)\s+meaning\s+'([^']+)'", re.IGNORECASE)
    m = pattern.search(text)
    if not m:
        return text
    prefix, lo, lo_lab, hi, hi_lab = m.groups()
    repl = f"{prefix}{hi} meaning '{hi_lab}' and {lo} meaning '{lo_lab}'"
    return pattern.sub(repl, text, count=1)


def paraphrase_light(text: str) -> str:
    out = text
    reps = [
        (r'^On a scale of', 'Using a scale of'),
        (r'how important is', 'how important would you say is'),
        (r'how often do you', 'how often would you say you'),
        (r'how do you view', 'how would you rate'),
        (r'what are your views on', 'what is your view of'),
        (r'do you agree or disagree with the following statement', 'to what extent do you agree with the following statement'),
        (r'how justifiable is it to', 'how justifiable would you consider it to'),
    ]
...
def apply_perturbation(question: str, perturbation: str) -> str:
    if perturbation == 'original':
        return question
    if perturbation == 'reverse_anchor_order':
        return reverse_anchor_order(question)
    if perturbation == 'paraphrase_light':
        return paraphrase_light(question)
...
def main() -> None:
    ...
    mode = cfg.benchmark.get('prompt_mode', cfg.benchmark.get('prompt_modes', ['interview_direct_distribution'])[0])
    model = cfg.api['model']
    questions = cfg.benchmark.get('questions', cfg.benchmark.get('question_ids'))
    if questions is None:
        raise KeyError('benchmark.questions or benchmark.question_ids is required')
    qids = list(questions)[:6] if args.test_run else list(questions)
    groups = list(cfg.benchmark['groups'])[:3] if args.test_run else list(cfg.benchmark['groups'])
    perturbations = list(cfg.benchmark.get('perturbations', ['original']))

    if args.resume_run_dir:
        run_dir = Path(args.resume_run_dir)
    else:
        stamp = time.strftime('%Y%m%d_%H%M%S')
        suffix = (args.run_name or f"{model.split('/')[-1]}-{mode}-invariance").replace(' ', '_')
        run_dir = Path(cfg.paths['outputs_dir']) / 'runs' / f'invariance_{stamp}_{suffix}'
    ...
    total_rows = len(subset) * len(perturbations)
    logger.info('Resume status: already_completed=%s total_rows=%s', len(completed), total_rows)
    ...
    try:
        ...
        with open(raw_path, 'a', encoding='utf-8') as rawf, open(error_path, 'a', encoding='utf-8') as errf, open(predictions_path, 'a', encoding='utf-8', newline='') as predf:
            ...
            for row in tqdm(subset.to_dict(orient='records'), desc=f'{model}:{mode}:invariance'):
                qid = str(row['QUESTION_ID'])
                qmeta = meta[qid]
                lo, hi = int(qmeta['answer_scale_min']), int(qmeta['answer_scale_max'])
                for perturbation in perturbations:
                    key=(qid, str(row['PARTICIPANT_ID']), mode, perturbation)
                    if key in completed:
                        continue
                    question_text = apply_perturbation(str(qmeta['question']), perturbation)
                    prompt = build_distribution_prompt(mode, question_text, str(row['Continent']), str(row['Urban_Rural']), str(row['Education']), lo, hi)
                    ...
                    try:
                        ...
                        sampled_score = sample_score(option_probs, str(row['PARTICIPANT_ID']), qid, mode, perturbation, int(cfg.benchmark.get('sampling_seed_base', 17)))
                        out = {
                            'QUESTION_ID': qid,
                        ...
                        errf.write(json.dumps({'QUESTION_ID': qid, 'PARTICIPANT_ID': row['PARTICIPANT_ID'], 'GROUP_KEY': row['GROUP_KEY'], 'prompt_mode': mode, 'perturbation': perturbation, 'model': model, 'prompt': prompt, 'error': str(e), 'raw_text': raw_text}, ensure_ascii=False) + '\n')
                        errf.flush()
                        logger.exception('Row failed | qid=%s participant=%s perturbation=%s', qid, row['PARTICIPANT_ID'], perturbation)
                        if args.fail_on_error:
                            raise

        pred_df = robust_read_predictions(predictions_path)
        summaries = {}
        for perturbation, pdf in pred_df.groupby('perturbation'):
            summaries[perturbation] = evaluate(pdf.copy(), meta)
        with open(run_dir / 'summary_by_perturbation.json', 'w', encoding='utf-8') as f:
            json.dump(summaries, f, ensure_ascii=False, indent=2)
        # compare against original at question level
        if set(pred_df['perturbation'].unique()) >= {'original'}:
            rows=[]
            orig = pred_df[pred_df['perturbation']=='original']
            for perturbation, pdf in pred_df.groupby('perturbation'):
                if perturbation == 'original':
                    continue
                for qid in sorted(orig['QUESTION_ID'].unique()):
                    oq = orig[orig['QUESTION_ID']==qid]
                    pq = pdf[pdf['QUESTION_ID']==qid]
                    if oq.empty or pq.empty:
                        continue
                    lo, hi = int(meta[qid]['answer_scale_min']), int(meta[qid]['answer_scale_max'])
                    ow1 = wasserstein_distance(((oq['HUMAN_SCORE']-lo)/(hi-lo)).tolist(), ((oq['MODEL_SCORE']-lo)/(hi-lo)).tolist())
                    pw1 = wasserstein_distance(((pq['HUMAN_SCORE']-lo)/(hi-lo)).tolist(), ((pq['MODEL_SCORE']-lo)/(hi-lo)).tolist())
                    rows.append({'perturbation': perturbation, 'QUESTION_ID': qid, 'category': meta[qid]['category'], 'original_w1': ow1, 'perturbed_w1': pw1, 'delta_w1': pw1-ow1})
            pd.DataFrame(rows).to_csv(run_dir/'question_delta_vs_original.tsv', sep='\t', index=False)
        logger.info('Saved invariance outputs to %s', run_dir)
    finally:
